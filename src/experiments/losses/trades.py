"""
Implementation of the TRADES loss function for adversarial training.
...
def trades_loss(
    ...
    if distance == "l_inf":
        for _ in range(perturb_steps):
            ...
            x_adv = torch.clamp(x_adv, clip_values[0], clip_values[1])
    elif distance == "l_2":
        ...
        for _ in range(perturb_steps):
            ...
            delta.data.clamp_(clip_values[0], clip_values[1]).sub_(x_natural)
    ...
    x_adv = Variable(
        torch.clamp(x_adv, clip_values[0], clip_values[1]), requires_grad=False
    )
    # zero gradient
    optimizer.zero_grad()
    ...
    if extra_outputs:
        return loss, x_adv, loss_robust, loss_natural
    else:
        return loss
